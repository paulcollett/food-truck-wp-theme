...
class BraceMenu {
  ...
  function __construct() {
    ...
    $this->uri = get_template_directory_uri() . '/functions/vendor/brace-menu';
    $this->post_type = 'brace-menu';
    $this->menu_slug = 'brace-menu';
    $this->label = 'Menus';
    $this->label_singular = 'Menu';
    ...
    // custom brace filter to add layouts to page post type
    add_filter('acf/page/layouts/add', array( $this, 'add_page_layout') );

  }

  function add_page_layout($added_layouts) {
      $added_layouts[] = array(
          'index' => 3,
          'data' => array(@json_decode(file_get_contents($this->uri . '/templates/layout.json'), true)),
      );
